...
class AuthorCreateViewTest(TestCase):
    ...
    def test_create_url_for_logged_in_with_permission(self):
        login = self.client.login(username='testuser2', password='2HJ1vRV0Z&3iD')
        response = self.client.get(reverse('author-create'))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context['form'].initial['date_of_death'], self.default_date)
    ...
    def test_add_new_author_btn_is_visible_for_logged_in_with_permission(self):
        login = self.client.login(username='testuser2', password='2HJ1vRV0Z&3iD')
        response = self.client.get(reverse('authors'))
        self.assertEqual(response.status_code, 200)
        self.assertIs(response.context['user'].has_perm('catalog.add_author'), True)
        # Check if the author list page has a 'add new' button visible for the user
        self.assertContains(response,'Add New')

    def test_add_new_author_btn_not_visible_for_logged_in_but_not_correct_permission(self):
        login = self.client.login(username='testuser1', password='1X<ISRUkw+tuK')
        response = self.client.get(reverse('authors'))
        self.assertEqual(response.status_code, 200)
        self.assertIs(response.context['user'].has_perm('catalog.add_author'), False)
        # Check if the author list page has a 'add new' button visible for the user
    ...
    def test_create_new_author(self):
        ...
        response = self.client.post(
            reverse('author-create'), {
                'first_name':'Benjamin',
                'last_name':'Honorio',
                'date_of_birth':datetime.date.fromisoformat('1993-08-03'),
                'date_of_death':''})
